...
- **조인과 ORDER BY**
  ...
  - 드라이빙 테이블만 먼저 정렬하는 경우에도 인덱스로 정렬을 해결하지 못하면 `Extra`에는 `Using filesort`가 표시될 수 있다.
...
`HAVING`은 `WHERE`와 달리 그룹핑과 집계가 끝난 뒤 그룹 결과에 적용된다. 인덱스는 개별 레코드의 값을 빠르게 찾거나 정렬된 순서로 읽기 위한 구조이므로, `COUNT(*) > 1`이나 `AVG(age) >= 30` 같은 집계 결과 자체를 인덱스로 바로 찾을 수는 없다.

그래서 행 단위로 먼저 줄일 수 있는 조건은 가능하면 `WHERE`에 두는 것이 좋다. `WHERE`에서 레코드 수를 줄이면 그 뒤의 `GROUP BY`와 `HAVING` 처리 대상도 함께 줄어든다.

단, `HAVING region = 'SEOUL'`처럼 그룹 컬럼에 대한 단순 조건은 옵티마이저가 `WHERE` 조건처럼 밀어 넣어 처리할 수도 있다. 여기서 말하는 "인덱스를 사용하기 어렵다"는 주로 집계 함수 결과나 그룹핑 이후 결과에 대한 조건을 의미한다.
...
이 방식이 사용되더라도 실행 계획의 `Extra` 컬럼에 "인덱스 스캔으로 그룹핑했다"는 전용 메시지가 반드시 표시되는 것은 아니다.

확인할 때는 `key`가 `GROUP BY` 컬럼 순서와 맞는 인덱스를 선택했는지, 그리고 `Extra`에 `Using temporary`나 `Using filesort`가 없는지를 함께 본다.

또한 `Using index`는 `GROUP BY`를 인덱스로 처리했다는 뜻이 아니라, 필요한 컬럼을 인덱스만으로 읽은 **커버링 인덱스**라는 뜻이다. 따라서 인덱스 순서로 그룹핑하더라도 테이블 레코드를 추가로 읽어야 하면 `Using index`가 표시되지 않을 수 있다.
...
`DISTINCT`는 크게 두 방식으로 처리될 수 있다.

첫 번째는 인덱스를 이용하는 방식이다. `DISTINCT` 대상 컬럼이 인덱스의 왼쪽부터 이어지는 순서와 맞으면, MySQL은 인덱스를 순서대로 읽으면서 같은 값의 반복을 건너뛸 수 있다.
이 경우 `GROUP BY` 최적화와 비슷하게 처리되며, 임시 테이블 없이 중복 제거가 가능하다.

두 번째는 중복 여부를 따로 기억하는 방식이다. 인덱스 순서로 중복을 제거할 수 없으면 MySQL은 이미 나온 값의 조합을 저장해두고, 같은 조합이 다시 나오면 버려야 한다.

이때 내부 임시 테이블이나 임시 자료구조가 사용될 수 있다.

여기서 헷갈리기 쉬운 점은 `Extra`의 `Using temporary`가 "MySQL이 내부에서 잠깐 저장하는 모든 작업"을 전부 표시하는 값은 아니라는 것이다.

`Using temporary`는 보통 `SELECT`의 중간 결과를 별도 임시 테이블에 담아 처리해야 할 때 표시된다. 예를 들어 `GROUP BY`와 `ORDER BY` 기준이 다르거나, `DISTINCT`와 `ORDER BY`가 함께 사용되어 결과를 따로 모아야 하는 경우에 표시될 수 있다.

반면 `COUNT(DISTINCT region)`은 최종 결과가 한 행이다. MySQL은 레코드를 읽으면서 지금까지 본 `region` 값을 기억해두고, 처음 보는 값이면 개수에 반영하고 이미 본 값이면 버린다. 예를 들어 `SEOUL`을 처음 보면 세고, 다음 `SEOUL`은 세지 않는 식이다.

이런 "이미 본 값 목록"은 중복인지 아닌지 확인하기 위해 집계 함수 내부에서 잠깐 사용하는 목록이다. 일반 `EXPLAIN`의 `Extra`는 이런 내부 목록까지 항상 `Using temporary`로 보여주지는 않는다.

그래서 `COUNT(DISTINCT ...)` 쿼리에서 `Extra`에 `Using temporary`가 없다는 말은 **중복 제거 비용이 없다는 뜻이 아니다.** 단지 실행 계획에 `Using temporary`라는 문구가 표시되지 않았다는 뜻으로 이해하는 것이 안전하다.
...
이 쿼리는 `idx_order_distinct_region (region)` 인덱스가 있으므로 루스 인덱스 스캔 후보가 될 수 있다.

`COUNT(DISTINCT region)`은 같은 `region` 값이 몇 번 반복되는지는 중요하지 않고, 서로 다른 `region` 값이 몇 개인지만 필요하다. 인덱스가 `region` 순서로 정렬되어 있으면 MySQL은 같은 `region` 값을 모두 하나씩 세지 않고, 각 `region` 그룹의 대표 위치만 확인한 뒤 다음 `region` 값으로 건너뛸 수 있다. 

이때 `Extra`에 `Using index for group-by`가 표시될 수 있다.

다만 집합 함수 안의 `DISTINCT`가 항상 루스 인덱스 스캔으로 처리되는 것은 아니다. `DISTINCT` 대상 컬럼이 인덱스의 왼쪽부터 이어지는 순서와 맞지 않거나, 여러 컬럼에 대해 서로 다른 `COUNT(DISTINCT ...)`를 동시에 수행하거나, 추가 조건 때문에 인덱스만으로 중복 제거가 어려우면 내부 임시 구조나 일반 인덱스 스캔을 사용할 수 있다.
...
- `DISTINCT` 대상 컬럼이 인덱스 순서와 맞으면 루스 인덱스 스캔 후보가 될 수 있다.
...
4. 드라이빙 테이블만 먼저 정렬하는 경우 `Extra`에는 무엇이 표시될 수 있을까?

확인 포인트:

- `STRAIGHT_JOIN`을 사용하면 `quiz_member`를 먼저 읽도록 고정할 수 있다.
- 드라이빙 테이블 컬럼으로 정렬하면 조인 전에 정렬 대상을 줄일 수 있다.
- 드라이빙 테이블만 정렬하더라도 인덱스로 정렬을 해결하지 못하면 `Using filesort`가 표시될 수 있다.
...
즉, 드라이빙 테이블만 먼저 정렬하는 경우에는 보통 `Using filesort`만 표시될 수 있다. 만약 `ORDER BY m.name`을 만족하는 인덱스가 있어서 인덱스 순서대로 읽을 수 있다면 `Using filesort`도 표시되지 않을 수 있다.
...
- `ORDER BY m.name`: 드라이빙 테이블만 정렬 가능, 필요하면 `Using filesort`
- `ORDER BY r.reservation_time`: 조인 결과를 임시 테이블에 저장한 뒤 정렬할 가능성 높음, `Using temporary; Using filesort`
...
- 인덱스 스캔을 이용한 `GROUP BY`는 `Extra`에 전용 메시지가 반드시 표시되지 않는다.
- `Using index`는 커버링 인덱스를 의미한다. 인덱스 스캔 `GROUP BY`의 필수 표시값은 아니다.
...
이때 `Extra`에 `Using index`가 반드시 떠야 하는 것은 아니다. `Using index`는 필요한 값을 인덱스만으로 읽었다는 커버링 인덱스 표시다. `SELECT` 절이나 조건 때문에 테이블 레코드를 추가로 읽어야 하면 인덱스 순서로 그룹핑하더라도 `Using index`는 표시되지 않을 수 있다.
